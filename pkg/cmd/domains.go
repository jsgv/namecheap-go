...
func NewCmdDomainsGetList() *cobra.Command {
	...
	cmd.Flags().StringVar(&opts.ListType, "listtype", "", "Possible values are ALL, EXPIRING, or EXPIRED. Default Value: ALL")
	cmd.Flags().StringVar(&opts.SearchTerm, "searchterm", "", "Keyword to look for in the domain list")
	cmd.Flags().StringVar(&opts.Page, "page", "", "Page to return Default Value: 1")
	cmd.Flags().StringVar(&opts.PageSize, "pagesize", "", "Page to return Default Value: 1")
